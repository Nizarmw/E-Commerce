...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (response.data.categoryId) {
          const relatedResponse = await axios.get(`${process.env.REACT_APP_API_URL}/products/?category=${response.data.categoryId}&limit=8`);
    ...
    const fetchReviews = async () => {
      ...
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/products/${id}/reviews/`);
